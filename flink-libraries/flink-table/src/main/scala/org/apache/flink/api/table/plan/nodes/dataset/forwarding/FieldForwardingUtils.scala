...
package org.apache.flink.api.table.plan.nodes.dataset.forwarding
    ...
    def chooseWrapper(typeInformation: TypeInformation[_]): (Int) => String = {
      typeInformation match {
        case composite: CompositeType[_] =>
          ...
          if (calcProgram.isDefined && typeFieldList.toSet == projectFieldList) {
            compositeTypeField(projectFieldList)
          } else {
